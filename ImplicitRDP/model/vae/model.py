...
from ImplicitRDP.model.common.shape_util import get_output_shape
from ImplicitRDP.model.vector_quantize_pytorch.residual_vq import ResidualVQ
from ImplicitRDP.model.vae.distributions import DiagonalGaussianDistribution
from ImplicitRDP.model.vae.utils import *
...
class UpSamplerCNN(nn.Module):
    def __init__(self,
        ...
        super(UpSamplerCNN, self).__init__()
        
        assert len(output_paddings) == layer_num + 1, "paddings must be the same length as layer_num + 1"
        layers = []
        for i in range(layer_num):
            if i == 0:
                layers.append(nn.ConvTranspose1d(input_dim, hidden_dim, kernel_size=5, stride=2, padding=2, output_padding=output_paddings[i]))
            else:
                layers.append(nn.ConvTranspose1d(hidden_dim, hidden_dim, kernel_size=5, stride=2, padding=2, output_padding=output_paddings[i]))
            layers.append(nn.ReLU())
        layers.append(nn.ConvTranspose1d(hidden_dim, output_dim, kernel_size=5, stride=2, padding=2, output_padding=output_paddings[-1]))
...
class EncoderCNN(nn.Module):
    def __init__(self,
        ...
        for i in range(layer_num):
            if i == 0:
                self.layers.append(nn.Conv1d(input_dim, hidden_dim, kernel_size=5, stride=2, padding=2))
            else:
                self.layers.append(nn.Conv1d(hidden_dim, hidden_dim, kernel_size=5, stride=2, padding=2))
            self.layers.append(nn.ReLU())
        self.layers.append(nn.Conv1d(hidden_dim, output_dim, kernel_size=5, stride=2, padding=2))
        self.encoder = nn.Sequential(*self.layers)

        if use_upsampler:
            upsampler_output_paddings = self.get_upsampler_output_paddings(input_horizon, input_dim)
            self.upsampler = UpSamplerCNN(
                input_dim=output_dim, output_dim=output_dim, hidden_dim=hidden_dim, layer_num=layer_num, output_paddings=upsampler_output_paddings
            )
        
        self.apply(weights_init_encoder)
    
    def get_upsampler_output_paddings(self, input_horizon, input_dim):
        x = torch.zeros(1, input_dim, input_horizon)
        upsampler_output_paddings = []
        stride = 2
        padding = 2
        dilation = 1
        kernel_size = 5
        for i in range(len(self.layers)):
            ...
            if isinstance(self.layers[i], nn.Conv1d):
                L_out = x.shape[-1]
                upsampler_padding = L_in - ((L_out - 1) * stride - 2 * padding + dilation * (kernel_size - 1) + 1)
                upsampler_output_paddings.append(upsampler_padding)
        return upsampler_output_paddings
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, global_cond, temporal_cond):
        if self.use_upsampler:
            global_cond = global_cond.reshape(global_cond.size(0), -1, self.global_cond_dim)
            global_cond = global_cond[..., :temporal_cond.size(1), :]
        else:
...
class VAE:
    def __init__(
        ...
        vqvae_groups=4,
        kl_multiplier=1e-6,
        sample_first=True, # sample distribution first
        eval=True,
        device="cuda",
        load_dir=None,
        ...
        if self.use_rnn_decoder:
            ...
            for extented_obs_key in self.extented_obs_keys:
                assert len(shape_meta['extended_obs'][extented_obs_key]['shape']) == 1, \
                    "Only support 1d extended obs for RNN decoder"
            self.rnn_temporal_cond_dim = sum([shape_meta['extended_obs'][extented_obs_key]['shape'][-1] for extented_obs_key in self.extented_obs_keys])
        self.use_vq = use_vq
        ...
        if self.use_conv_encoder:
            self.encoder = EncoderCNN(
                input_dim=self.input_dim_w, input_horizon=self.input_dim_h, output_dim=n_latent_dims, hidden_dim=conv_latent_dims, layer_num=conv_layer_num, use_upsampler=use_upsampler
            ).to(self.device)
        else:
            self.encoder = MLP(
        ...
        if len(output_shape) == 1:
            ...
            if use_upsampler:
                decoder_n_latent_dims = output_shape[-1]
            else:
                decoder_n_latent_dims = np.multiply(*output_shape)
            self.downsampled_input_h = output_shape[0]
        
        if use_upsampler:
            assert self.use_conv_encoder and self.use_rnn_decoder, \
                "UpSampler is only supported for conv encoder and rnn decoder"
            assert self.downsampled_input_h == self.input_dim_h, \
                "Downsampled input horizon must be equal to input horizon when using upsampler"
        self.use_upsampler = use_upsampler
        ...
        if self.use_rnn_decoder:
            self.decoder = DecoderRNN(global_cond_dim=decoder_n_latent_dims, temporal_cond_dim=self.rnn_temporal_cond_dim,
                                      ...
                                      layer_num=rnn_layer_num, use_upsampler=use_upsampler).to(self.device)
        else:
            self.decoder = MLP(
    ...
    def quant_state_with_vq(self, state):
        ...
        state_vq = state_vq.reshape(batch_size, -1) # N, T, A -> N, (T A)
        vq_code = vq_code.reshape(batch_size, -1)
        vq_loss_state = torch.sum(vq_loss_state)

        return state_vq, vq_code, vq_loss_state

    def quant_state_without_vq(self, state, return_posterior=False):
        batch_size = state.size(0)
        if len(state.shape) == 2:
    ...
    def postprocess_quant_state_without_vq(self, state_vq, return_posterior=False):
        state_vq = einops.rearrange(state_vq, "N (T A) -> N A T", T=self.downsampled_input_h)
        if self.sample_first:
            posterior = None
        else:
            posterior = DiagonalGaussianDistribution(state_vq)
            state_vq = posterior.sample()
    ...
    def compute_loss_and_metric(self, batch):
        ...
        if self.use_vq:
            ...
            state_vq, posterior_1 = self.quant_state_without_vq(state_rep, return_posterior=True)
            state_vq, posterior_2 = self.postprocess_quant_state_without_vq(state_vq, return_posterior=True)
            posterior = posterior_1 if posterior_1 is not None else posterior_2
