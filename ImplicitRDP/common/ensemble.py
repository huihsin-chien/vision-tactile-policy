    ...
    def _weighted_average_action(self, actions, weights):
        ...
        if rotation_dim_list is not None:
            for rotation_dim in rotation_dim_list:
                ...
                avg_rotation = R.from_matrix(ortho6d_to_rotation_matrix_batch(actions[0:, rotation_dim])[0]).as_quat()
                cumulative_weight = weights[0]
                for i in range(1, len(weights)):
                    start_rotation = avg_rotation
                    end_rotation = R.from_matrix(ortho6d_to_rotation_matrix_batch(actions[i:, rotation_dim])[0]).as_quat()
