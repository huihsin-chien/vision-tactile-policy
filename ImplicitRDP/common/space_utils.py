...
try:
    ...
    logger.warning("ROS2 not installed, related functions not available")
...
def ortho6d_to_rotation_matrix_batch(ortho6d: np.ndarray) -> np.ndarray:
    """
    Compute rotation matrix from ortho6d representation
    """
    x_raw = ortho6d[:, 0:3]  # batch * 3
    y_raw = ortho6d[:, 3:6]  # batch * 3
    x = normalize_vector_batch(x_raw)  # batch * 3
    z = np.cross(x, y_raw)  # batch * 3
    z = normalize_vector_batch(z)  # batch * 3
...
def normalize_vector_batch(v: np.ndarray) -> np.ndarray:
    """
...
def pose_9d_to_pose_6d(pose: np.ndarray) -> np.ndarray:
    """
    Convert 9D state to 6D state
    :param pose: np.ndarray (9,), (x, y, z, rx1, rx2, rx3, ry1, ry2, ry3)
    :return: np.ndarray (6,), (x, y, z, rx, ry, rz)
    """
    pose_6d = matrix4x4_to_pose_6d(pose_3d_9d_to_homo_matrix_batch(pose[None])[0])
    return pose_6d
...
def pose_9d_to_pose_6d_batch(pose: np.ndarray) -> np.ndarray:
    """
    Convert 9D state to 6D state
    :param pose: np.ndarray (N, 9), (x, y, z, rx1, rx2, rx3, ry1, ry2, ry3)
    :return: np.ndarray (N, 6), (x, y, z, rx, ry, rz)
    """
    pose_6d_list = []    
    for i in range(pose.shape[0]):
        pose_6d_list.append(pose_9d_to_pose_6d(pose[i]))
    return np.stack(pose_6d_list, axis=0)

def pose_6d_to_pose_9d_batch(pose: np.ndarray) -> np.ndarray:
    """
    Convert 6D state to 9D state
    :param pose: np.ndarray (N, 6), (x, y, z, rx, ry, rz)
    :return: np.ndarray (N, 9), (x, y, z, rx1, rx2, rx3, ry1, ry2, ry3)
    """
    pose_9d_list = []
    for i in range(pose.shape[0]):
        pose_9d_list.append(pose_6d_to_pose_9d(pose[i]))
    return np.stack(pose_9d_list, axis=0)
...
def pose_3d_9d_to_homo_matrix_batch(pose: np.ndarray) -> np.ndarray:
    ...
    mat[:, :3, 3] = pose[:, :3]
    if pose.shape[1] == 9:
        mat[:, :3, :3] = ortho6d_to_rotation_matrix_batch(pose[:, 3:9])
    return mat
...
def normalize_vector_batch_torch(v: torch.Tensor) -> torch.Tensor:
    """
    Normalize a vector (batch, 3)
    """
    v_mag = torch.norm(v, dim=1, keepdim=True)  # batch * 1
    v_mag = torch.maximum(v_mag, torch.tensor(1e-8))
    v = v / v_mag
    return v

def ortho6d_to_rotation_matrix_batch_torch(ortho6d: torch.Tensor) -> torch.Tensor:
    """
    Compute rotation matrix from ortho6d representation
    """
    x_raw = ortho6d[:, 0:3]  # batch * 3
    y_raw = ortho6d[:, 3:6]  # batch * 3
    x = normalize_vector_batch_torch(x_raw)  # batch * 3
    z = torch.cross(x, y_raw)  # batch * 3
    z = normalize_vector_batch_torch(z)  # batch * 3
...
def pose_3d_9d_to_homo_matrix_batch_torch(pose: torch.Tensor) -> torch.Tensor:
    """
    Convert 3D / 9D states to 4x4 matrix
    :param pose: torch.Tensor (N, 9) or (N, 3)
    :return: torch.Tensor (N, 4, 4)
    """
    assert pose.shape[1] in [3, 9], "pose should be (N, 3) or (N, 9)"
    mat = torch.eye(4, device=pose.device)[None, :, :].repeat(pose.shape[0], 1, 1)
    mat[:, :3, 3] = pose[:, :3]
    if pose.shape[1] == 9:
        mat[:, :3, :3] = ortho6d_to_rotation_matrix_batch_torch(pose[:, 3:9])
    return mat

def homo_matrix_to_pose_9d_batch_torch(mat: torch.Tensor) -> torch.Tensor:
    """
    Convert 4x4 matrix to 9D state
    :param mat: torch.Tensor (N, 4, 4)
    :return: torch.Tensor (N, 9)
    """
    assert mat.shape[1:] == (4, 4), "mat should be (N, 4, 4)"
    pose = torch.zeros((mat.shape[0], 9), device=mat.device)
    pose[:, :3] = mat[:, :3, 3]
    pose[:, 3:9] = mat[:, :3, :2].swapaxes(1, 2).reshape(mat.shape[0], -1)
    return pose