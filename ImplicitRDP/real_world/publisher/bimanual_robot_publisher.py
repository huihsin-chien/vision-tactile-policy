...
from ImplicitRDP.common.data_models import BimanualRobotStates, ForceSensorMessage, Arrow
from ImplicitRDP.common.space_utils import pose_7d_to_4x4matrix, matrix4x4_to_pose_6d, pose_7d_to_pose_6d
...
class BimanualRobotPublisher(Node):
    ...
    def __init__(self,
        ...
        # Force monitoring web server setup
        if self.vis_wrench:
            self.setup_force_monitor_server()

    def setup_force_monitor_server(self):
        """Setup FastAPI server for force monitoring"""
        self.websocket_clients = set()
        self.app = FastAPI()
        
        # Mount static files directory
        static_dir = os.path.join(os.path.dirname(__file__), 'force_monitor', 'static')
        self.app.mount("/static", StaticFiles(directory=static_dir), name="static")
        
        # Read HTML template
        template_path = os.path.join(os.path.dirname(__file__), 'force_monitor', 'template.html')
        try:
        ...
        async def websocket_endpoint(websocket: WebSocket):
            ...
            try:
                ...
                if not hasattr(self, 'force_queue'):
                    self.force_queue = queue.Queue()
                
                while True:
                    # Check for new data in queue
                    try:
                        data = self.force_queue.get_nowait()
                        await websocket.send_text(json.dumps(data))
                    except:
                        pass  # Continue when queue is empty
                    
                    await asyncio.sleep(0.01)  # Control sending frequency
            except Exception as e:
                logger.debug(f"WebSocket connection disconnected: {e}")
            finally:
        ...
        logger.info("Force monitoring web server started, access at: http://0.0.0.0:8000")
    
    def broadcast_force_data_sync(self, left_wrench, right_wrench):
        """Synchronous version of force data broadcasting"""
        if not hasattr(self, 'force_queue'):
            self.force_queue = queue.Queue()
        ...
        try:
            self.force_queue.put_nowait(data)
        except:
            pass  # Ignore when queue is full
    ...
    def timer_callback(self):
        ...
        if self.vis_wrench:
            # Send complete 6-DOF wrench data (force + torque) for both arms
            left_wrench = robot_states.leftRobotTCPWrench
            right_wrench = robot_states.rightRobotTCPWrench

            # Put data into queue for async processing
            if self.websocket_clients:
                self.broadcast_force_data_sync(left_wrench, right_wrench)
