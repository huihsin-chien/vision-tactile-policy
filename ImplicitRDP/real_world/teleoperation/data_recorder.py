...
from ImplicitRDP.common.data_models import (SensorMessage, SensorMessageList)
...
class DataRecorder(Node):
    ...
    def __init__(self,
        ...
        self.ts = ApproximateTimeSynchronizer(self.subscribers, queue_size=25, slop=0.1,
    ...
    def vis_wrench(self):
        force = [np.linalg.norm(sensor_msg.rightRobotTCPWrench[:3]) for sensor_msg in self.sensor_msg_list.sensorMessages]
        plt.figure()
        plt.plot(force)
        plt.title("Right Robot TCP Wrench Force")
        plt.xlabel("Time")
        plt.ylabel("Force (N)")
        plt.grid()
        plt.show()
    ...
    def save(self):
        ...
        with open(self.save_path, 'wb') as f:
            data = {
                'sensor_message_list': self.sensor_msg_list,
            }
            pickle.dump(data, f)
