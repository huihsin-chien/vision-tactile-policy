...
from ImplicitRDP.common.space_utils import (matrix4x4_to_pose_6d, pose_7d_to_4x4matrix,
                                                          pose_6d_to_pose_7d, pose_6d_to_4x4matrix)
from ImplicitRDP.real_world.real_world_transforms import RealWorldTransforms
from ImplicitRDP.common.data_models import UnityMes, BimanualRobotStates, RobotControlMode
...
class TeleopServer:
    ...
    def setup_routes(self):
        ...
        async def get_controller_type():
            return {"controller_type": "teleop"}

        @self.app.get('/get_tracking_state')
        async def get_tracking_state() -> Dict[str, bool]:
            return {
    ...
    def calc_relative_target(self, pos_from_unity: np.ndarray, is_left: bool) -> np.ndarray:
        ...
        # Displacement in Unity relative to the recorded starting position in Unity
        # Add to the recorded real starting position
        target = np.zeros(7)
    ...
    def process_cmd(self):
        while True:
            ...
            if self.gripper_interval_count == 0:
                ...
                if self.left_tracking_state and self.is_arm_enable('left'):
                ...
                if self.left_tracking_state and self.is_arm_enable('right'):
            ...
            if self.left_tracking_state:
                ...
                if self.robot_control_mode == RobotControlMode.left_arm_3D_translation:
                ...
                elif self.robot_control_mode == RobotControlMode.left_arm_6DOF:
                    ...
                    raise ValueError(f"Unsupported teleoperation mode: {self.robot_control_mode}")
            ...
            if self.right_tracking_state:
                ...
                if self.robot_control_mode == RobotControlMode.right_arm_6DOF:
                    ...
                    # clip action to avoid collision with table
                    # clip roll
                    right_target_6d_in_world[3] = np.clip(right_target_6d_in_world[3], -np.pi, -np.pi)
                    # clip pitch
